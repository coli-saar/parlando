---
title: Migrating Parlando 0.3.0 to 0.4.0
---

# Migrating Parlando 0.3.0 to 0.4.0

Parlando 0.4.0 changes game construction, agent factories, participant clients, experiment
configuration, and the database. Upgrade the code and a copy of the database before starting a
0.4.0 server against production data.
...
Upgrade the Python SDK and use the logger supplied through its `Context` when the policy needs
session diagnostics.
...
## Rebuild the participant application

Update the Parlando React package, rebuild the participant application, and run its tests. Games
that use `ParticipantApp` do not need to manage Parlando's network routes. If a project bypasses the
client package and implements that lower-level protocol itself, migrate it against the 0.4.0 source
and protocol tests before deploying; the manual does not treat those internals as a game-authoring
API.
