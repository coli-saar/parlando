...
Use `MicrophoneMuteButton`, `MicrophoneLevelMeter`, and `TranscriptionProgress` for active-game voice UI. Delegate microphone setup, transport, STT, TTS playback, credentials, and reconnection to the SDK/runtime. Do not add WebRTC, custom audio sockets/worklets, browser provider clients, or custom startup protocol handling.

Generated CSS must style both `ParticipantApp` lifecycle markup and the active game. Preserve accessibility attributes and visible focus/disabled states. A transformed microphone meter child needs block layout, full width/height, left transform origin, and a visible background.
