...
- Added task-oriented and API-reference migration guides for porting existing 0.2 games and automated migration agents to 0.3 without compatibility aliases.
...
- Replaced the public Rust and JavaScript authoring APIs with narrow game, server, agent, participant-client, and React session boundaries. Games now exchange typed actions, role-specific observations, and shared completion values without exposing authoritative state, generic presentation events, runtime configuration, storage, or protocol internals.
...
- Moved experiment configuration into database-backed dashboard editing. Listener port, database location, and client bundle remain process bootstrap settings. Provider credentials use the explicit game secret store, while exact Speechmatics and ElevenLabs destinations are immutable experiment settings.
...
- Simplified the JavaScript startup gate to show only the compiled game name and current platform tasks; removed arbitrary startup-label, eyebrow, setup-copy, and game-hint customization.
